...
public class ProcessingAncientFossilQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingAncientFossilQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/ancientfossilquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingAncientFossilQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingAncientFossilQuarryObjectEntity getAncientFossilQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingAncientFossilQuarryObjectEntity)
            return (ProcessingAncientFossilQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingAncientFossilQuarryObjectEntity ancientFossilQuarryObjectEntity = getAncientFossilQuarryObjectEntity(level, tileX, tileY);
        if (ancientFossilQuarryObjectEntity != null)
            return Recipes.streamRecipes(ancientFossilQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingAncientFossilQuarryObjectEntity ancientFossilQuarryObjectEntity = getAncientFossilQuarryObjectEntity(level, tileX, tileY);
        if (ancientFossilQuarryObjectEntity != null)
            return ((ancientFossilQuarryObjectEntity.getExpectedResults()).crafts < 10 && (ancientFossilQuarryObjectEntity
                    .isFuelRunning() || ancientFossilQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingAncientFossilQuarryObjectEntity ancientFossilQuarryObjectEntity = getAncientFossilQuarryObjectEntity(level, tileX, tileY);
        if (ancientFossilQuarryObjectEntity != null)
            return ancientFossilQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingAncientFossilQuarryObjectEntity ancientFossilQuarryObjectEntity = getAncientFossilQuarryObjectEntity(level, tileX, tileY);
        if (ancientFossilQuarryObjectEntity != null)
            return ancientFossilQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingAncientFossilQuarryObjectEntity ancientFossilQuarryObjectEntity = getAncientFossilQuarryObjectEntity(level, tileX, tileY);
        if (ancientFossilQuarryObjectEntity != null)
            return (ancientFossilQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingAncientFossilQuarryObjectEntity ancientFossilQuarryObjectEntity = getAncientFossilQuarryObjectEntity(level, tileX, tileY);
        if (ancientFossilQuarryObjectEntity != null) {
            Inventory inventory = ancientFossilQuarryObjectEntity.getInventory();
            if (inventory != null && ancientFossilQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, ancientFossilQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
