...
public class ProcessingTungstenQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingTungstenQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/tungstenquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingTungstenQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingTungstenQuarryObjectEntity getTungstenQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingTungstenQuarryObjectEntity)
            return (ProcessingTungstenQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingTungstenQuarryObjectEntity tungstenQuarryObjectEntity = getTungstenQuarryObjectEntity(level, tileX, tileY);
        if (tungstenQuarryObjectEntity != null)
            return Recipes.streamRecipes(tungstenQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingTungstenQuarryObjectEntity tungstenQuarryObjectEntity = getTungstenQuarryObjectEntity(level, tileX, tileY);
        if (tungstenQuarryObjectEntity != null)
            return ((tungstenQuarryObjectEntity.getExpectedResults()).crafts < 10 && (tungstenQuarryObjectEntity
                    .isFuelRunning() || tungstenQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingTungstenQuarryObjectEntity tungstenQuarryObjectEntity = getTungstenQuarryObjectEntity(level, tileX, tileY);
        if (tungstenQuarryObjectEntity != null)
            return tungstenQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingTungstenQuarryObjectEntity tungstenQuarryObjectEntity = getTungstenQuarryObjectEntity(level, tileX, tileY);
        if (tungstenQuarryObjectEntity != null)
            return tungstenQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingTungstenQuarryObjectEntity tungstenQuarryObjectEntity = getTungstenQuarryObjectEntity(level, tileX, tileY);
        if (tungstenQuarryObjectEntity != null)
            return (tungstenQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingTungstenQuarryObjectEntity tungstenQuarryObjectEntity = getTungstenQuarryObjectEntity(level, tileX, tileY);
        if (tungstenQuarryObjectEntity != null) {
            Inventory inventory = tungstenQuarryObjectEntity.getInventory();
            if (inventory != null && tungstenQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, tungstenQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
