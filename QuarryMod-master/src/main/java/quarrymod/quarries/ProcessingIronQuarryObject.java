...
public class ProcessingIronQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingIronQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/ironquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingIronQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingIronQuarryObjectEntity getIronQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingIronQuarryObjectEntity)
            return (ProcessingIronQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingIronQuarryObjectEntity ironQuarryObjectEntity = getIronQuarryObjectEntity(level, tileX, tileY);
        if (ironQuarryObjectEntity != null)
            return Recipes.streamRecipes(ironQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingIronQuarryObjectEntity ironQuarryObjectEntity = getIronQuarryObjectEntity(level, tileX, tileY);
        if (ironQuarryObjectEntity != null)
            return ((ironQuarryObjectEntity.getExpectedResults()).crafts < 10 && (ironQuarryObjectEntity
                    .isFuelRunning() || ironQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingIronQuarryObjectEntity ironQuarryObjectEntity = getIronQuarryObjectEntity(level, tileX, tileY);
        if (ironQuarryObjectEntity != null)
            return ironQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingIronQuarryObjectEntity ironQuarryObjectEntity = getIronQuarryObjectEntity(level, tileX, tileY);
        if (ironQuarryObjectEntity != null)
            return ironQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingIronQuarryObjectEntity ironQuarryObjectEntity = getIronQuarryObjectEntity(level, tileX, tileY);
        if (ironQuarryObjectEntity != null)
            return (ironQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingIronQuarryObjectEntity ironQuarryObjectEntity = getIronQuarryObjectEntity(level, tileX, tileY);
        if (ironQuarryObjectEntity != null) {
            Inventory inventory = ironQuarryObjectEntity.getInventory();
            if (inventory != null && ironQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, ironQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
