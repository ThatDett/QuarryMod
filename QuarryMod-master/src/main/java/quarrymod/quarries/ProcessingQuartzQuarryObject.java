...
public class ProcessingQuartzQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingQuartzQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/quartzquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingQuartzQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingQuartzQuarryObjectEntity getQuartzQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingQuartzQuarryObjectEntity)
            return (ProcessingQuartzQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingQuartzQuarryObjectEntity quartzQuarryObjectEntity = getQuartzQuarryObjectEntity(level, tileX, tileY);
        if (quartzQuarryObjectEntity != null)
            return Recipes.streamRecipes(quartzQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingQuartzQuarryObjectEntity quartzQuarryObjectEntity = getQuartzQuarryObjectEntity(level, tileX, tileY);
        if (quartzQuarryObjectEntity != null)
            return ((quartzQuarryObjectEntity.getExpectedResults()).crafts < 10 && (quartzQuarryObjectEntity
                    .isFuelRunning() || quartzQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingQuartzQuarryObjectEntity quartzQuarryObjectEntity = getQuartzQuarryObjectEntity(level, tileX, tileY);
        if (quartzQuarryObjectEntity != null)
            return quartzQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingQuartzQuarryObjectEntity quartzQuarryObjectEntity = getQuartzQuarryObjectEntity(level, tileX, tileY);
        if (quartzQuarryObjectEntity != null)
            return quartzQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingQuartzQuarryObjectEntity quartzQuarryObjectEntity = getQuartzQuarryObjectEntity(level, tileX, tileY);
        if (quartzQuarryObjectEntity != null)
            return (quartzQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingQuartzQuarryObjectEntity quartzQuarryObjectEntity = getQuartzQuarryObjectEntity(level, tileX, tileY);
        if (quartzQuarryObjectEntity != null) {
            Inventory inventory = quartzQuarryObjectEntity.getInventory();
            if (inventory != null && quartzQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, quartzQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
