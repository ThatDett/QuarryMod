...
public class ProcessingMyceliumQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingMyceliumQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/myceliumquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingMyceliumQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingMyceliumQuarryObjectEntity getMyceliumQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingMyceliumQuarryObjectEntity)
            return (ProcessingMyceliumQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingMyceliumQuarryObjectEntity myceliumQuarryObjectEntity = getMyceliumQuarryObjectEntity(level, tileX, tileY);
        if (myceliumQuarryObjectEntity != null)
            return Recipes.streamRecipes(myceliumQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingMyceliumQuarryObjectEntity myceliumQuarryObjectEntity = getMyceliumQuarryObjectEntity(level, tileX, tileY);
        if (myceliumQuarryObjectEntity != null)
            return ((myceliumQuarryObjectEntity.getExpectedResults()).crafts < 10 && (myceliumQuarryObjectEntity
                    .isFuelRunning() || myceliumQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingMyceliumQuarryObjectEntity myceliumQuarryObjectEntity = getMyceliumQuarryObjectEntity(level, tileX, tileY);
        if (myceliumQuarryObjectEntity != null)
            return myceliumQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingMyceliumQuarryObjectEntity myceliumQuarryObjectEntity = getMyceliumQuarryObjectEntity(level, tileX, tileY);
        if (myceliumQuarryObjectEntity != null)
            return myceliumQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingMyceliumQuarryObjectEntity myceliumQuarryObjectEntity = getMyceliumQuarryObjectEntity(level, tileX, tileY);
        if (myceliumQuarryObjectEntity != null)
            return (myceliumQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingMyceliumQuarryObjectEntity myceliumQuarryObjectEntity = getMyceliumQuarryObjectEntity(level, tileX, tileY);
        if (myceliumQuarryObjectEntity != null) {
            Inventory inventory = myceliumQuarryObjectEntity.getInventory();
            if (inventory != null && myceliumQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, myceliumQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
