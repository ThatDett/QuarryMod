...
public class ProcessingGlacialQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingGlacialQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/glacialquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingGlacialQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingGlacialQuarryObjectEntity getGlacialQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingGlacialQuarryObjectEntity)
            return (ProcessingGlacialQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingGlacialQuarryObjectEntity glacialQuarryObjectEntity = getGlacialQuarryObjectEntity(level, tileX, tileY);
        if (glacialQuarryObjectEntity != null)
            return Recipes.streamRecipes(glacialQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingGlacialQuarryObjectEntity glacialQuarryObjectEntity = getGlacialQuarryObjectEntity(level, tileX, tileY);
        if (glacialQuarryObjectEntity != null)
            return ((glacialQuarryObjectEntity.getExpectedResults()).crafts < 10 && (glacialQuarryObjectEntity
                    .isFuelRunning() || glacialQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingGlacialQuarryObjectEntity glacialQuarryObjectEntity = getGlacialQuarryObjectEntity(level, tileX, tileY);
        if (glacialQuarryObjectEntity != null)
            return glacialQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingGlacialQuarryObjectEntity glacialQuarryObjectEntity = getGlacialQuarryObjectEntity(level, tileX, tileY);
        if (glacialQuarryObjectEntity != null)
            return glacialQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingGlacialQuarryObjectEntity glacialQuarryObjectEntity = getGlacialQuarryObjectEntity(level, tileX, tileY);
        if (glacialQuarryObjectEntity != null)
            return (glacialQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingGlacialQuarryObjectEntity glacialQuarryObjectEntity = getGlacialQuarryObjectEntity(level, tileX, tileY);
        if (glacialQuarryObjectEntity != null) {
            Inventory inventory = glacialQuarryObjectEntity.getInventory();
            if (inventory != null && glacialQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, glacialQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
