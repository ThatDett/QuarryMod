...
public class ProcessingCopperQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingCopperQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/copperquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingCopperQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingCopperQuarryObjectEntity getCopperQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingCopperQuarryObjectEntity)
            return (ProcessingCopperQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingCopperQuarryObjectEntity copperQuarryObjectEntity = getCopperQuarryObjectEntity(level, tileX, tileY);
        if (copperQuarryObjectEntity != null)
            return Recipes.streamRecipes(copperQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingCopperQuarryObjectEntity copperQuarryObjectEntity = getCopperQuarryObjectEntity(level, tileX, tileY);
        if (copperQuarryObjectEntity != null)
            return ((copperQuarryObjectEntity.getExpectedResults()).crafts < 10 && (copperQuarryObjectEntity
                    .isFuelRunning() || copperQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingCopperQuarryObjectEntity copperQuarryObjectEntity = getCopperQuarryObjectEntity(level, tileX, tileY);
        if (copperQuarryObjectEntity != null)
            return copperQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingCopperQuarryObjectEntity copperQuarryObjectEntity = getCopperQuarryObjectEntity(level, tileX, tileY);
        if (copperQuarryObjectEntity != null)
            return copperQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingCopperQuarryObjectEntity copperQuarryObjectEntity = getCopperQuarryObjectEntity(level, tileX, tileY);
        if (copperQuarryObjectEntity != null)
            return (copperQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingCopperQuarryObjectEntity copperQuarryObjectEntity = getCopperQuarryObjectEntity(level, tileX, tileY);
        if (copperQuarryObjectEntity != null) {
            Inventory inventory = copperQuarryObjectEntity.getInventory();
            if (inventory != null && copperQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, copperQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
