...
public class ProcessingCoalQuarryObject extends GameObject implements SettlementWorkstationObject {
    public GameTexture texture;

    public ProcessingCoalQuarryObject() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/coalquarry");
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new ProcessingCoalQuarryObjectEntity(level, x, y);
    }
    ...
    public ProcessingCoalQuarryObjectEntity getCoalQuarryObjectEntity(Level level, int tileX, int tileY) {
        ObjectEntity objectEntity = level.entityManager.getObjectEntity(tileX, tileY);
        if (objectEntity instanceof ProcessingCoalQuarryObjectEntity)
            return (ProcessingCoalQuarryObjectEntity)objectEntity;
        return null;
    }

    public Stream<Recipe> streamSettlementRecipes(Level level, int tileX, int tileY) {
        ProcessingCoalQuarryObjectEntity coalQuarryObjectEntity = getCoalQuarryObjectEntity(level, tileX, tileY);
        if (coalQuarryObjectEntity != null)
            return Recipes.streamRecipes(coalQuarryObjectEntity.techs);
        return Stream.empty();
    }
    ...
    public boolean canCurrentlyCraft(Level level, int tileX, int tileY, Recipe recipe) {
        ProcessingCoalQuarryObjectEntity coalQuarryObjectEntity = getCoalQuarryObjectEntity(level, tileX, tileY);
        if (coalQuarryObjectEntity != null)
            return ((coalQuarryObjectEntity.getExpectedResults()).crafts < 10 && (coalQuarryObjectEntity
                    .isFuelRunning() || coalQuarryObjectEntity.canUseFuel()));
        return false;
    }
    ...
    public InventoryRange getProcessingInputRange(Level level, int tileX, int tileY) {
        ProcessingCoalQuarryObjectEntity coalQuarryObjectEntity = getCoalQuarryObjectEntity(level, tileX, tileY);
        if (coalQuarryObjectEntity != null)
            return coalQuarryObjectEntity.getInputInventoryRange();
        return null;
    }
    ...
    public InventoryRange getProcessingOutputRange(Level level, int tileX, int tileY) {
        ProcessingCoalQuarryObjectEntity coalQuarryObjectEntity = getCoalQuarryObjectEntity(level, tileX, tileY);
        if (coalQuarryObjectEntity != null)
            return coalQuarryObjectEntity.getOutputInventoryRange();
        return null;
    }
    ...
    public ArrayList<InventoryItem> getCurrentAndFutureProcessingOutputs(Level level, int tileX, int tileY) {
        ProcessingCoalQuarryObjectEntity coalQuarryObjectEntity = getCoalQuarryObjectEntity(level, tileX, tileY);
        if (coalQuarryObjectEntity != null)
            return (coalQuarryObjectEntity.getCurrentAndExpectedResults()).items;
    ...
    public InventoryRange getFuelInventoryRange(Level level, int tileX, int tileY) {
        ProcessingCoalQuarryObjectEntity coalQuarryObjectEntity = getCoalQuarryObjectEntity(level, tileX, tileY);
        if (coalQuarryObjectEntity != null) {
            Inventory inventory = coalQuarryObjectEntity.getInventory();
            if (inventory != null && coalQuarryObjectEntity.fuelSlots > 0)
                return new InventoryRange(inventory, 0, coalQuarryObjectEntity.fuelSlots - 1);
        }
        return null;
    }
}
